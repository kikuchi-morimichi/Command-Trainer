...
　//document.addEventListener('DOMContentLoaded', () => {
                ...
                problems: [
                    {
                        ...
                        steps: [
                            ...
                            { id: "4-2", question: "Create a configuration file named 'server.conf'.", answers: ["touch server.conf"], successMessage: "Created 'server.conf'." },
    ...
    class WizardManager {
        ...
        renderConfirmationSummary() {
            ...
            if(addedSets.length > 0) {
                hasChanges = true;
                html += `<li><strong class="${CONFIG.CSS_CLASSES.SUCCESS}">Added sets:</strong> ${addedSets.map(k => this.state.stagedData[k].name).join(', ')}</li>`;
            }
            if(removedSets.length > 0) {
                hasChanges = true;
                html += `<li><strong class="${CONFIG.CSS_CLASSES.ERROR}">Removed sets:</strong> ${removedSets.map(k => this.state.initialData[k].name).join(', ')}</li>`;
            }
            ...
            modifiedSets.forEach(key => {
                ...
                if(initialProblems !== stagedProblems) {
                    hasChanges = true;
                    html += `<li><strong class="${CONFIG.CSS_CLASSES.HINT}">Modified set "${this.state.stagedData[key].name}":</strong> Problem count will change from ${initialProblems} to ${stagedProblems}.</li>`;
                }
            });
        ...
        _handleFileUpload(event) {
          ...
          const i18nMessages = I18N[this.appController.currentLang].messages;
          if (!file) return;
          ...
          reader.onload = (e) => {
            try {
              ...
              if (this.appController._validateCustomProblemData(customData, this.state.stagedData)) {
                Object.assign(this.state.stagedData, customData); // 読み込んだデータをマージ
                feedbackEl.innerHTML = `<span class="${CONFIG.CSS_CLASSES.SUCCESS}">${i18nMessages.WIZARD_UPLOAD_SUCCESS(file.name)}</span>`;
              }
            } catch (error) {
              console.error('Failed to load My Problems:', error);
              feedbackEl.innerHTML = `<span class="${CONFIG.CSS_CLASSES.ERROR}">${i18nMessages.JSON_PARSE_ERROR(error.message)}</span>`;
            } finally {
              event.target.value = '';
            }
          };
          reader.onerror = () => {
            feedbackEl.innerHTML = `<span class="${CONFIG.CSS_CLASSES.ERROR}">${i18nMessages.FILE_READ_ERROR}</span>`;
            event.target.value = '';
          };
    ...
    class AppController {
      ...
      commitWizardChanges(newData) {
          ...
          this.renderSidebar();
          this.startNewSession();
          alert('My Problems have been updated.');
      }
